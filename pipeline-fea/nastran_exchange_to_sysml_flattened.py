#!/usr/bin/env python3
"""
Flattened presentation-mode baseline exchange JSON -> SysML v2 text.

Purpose
-------
Convert the baseline exchange JSON into a flattened, diagram-friendly SysML v2
text file for renderers that do not display instance values well when part
definitions are shown separately.

This version uses the ORIGINAL input filename stem as the top-level visible
part name, so the diagram shows a root part like `test` rather than a derived
name like `test_presentation`.
...
def emit_sysml(exchange: dict[str, Any]) -> str:
    ...
    original_stem = (
        sim_id.get("sourceFileStem")
        or provenance.get("sourceFileStem")
        or sim_id.get("name")
        or "presentationView"
    )
    ...
    a("    baseline Nastran exchange JSON.")
    a("  */")
    a("")
    a(f"  part {root_name} {{")
    a("")
    ...
    a(f"      attribute version : String = {q(solver.get('version'))};")
    a(f"      attribute sol : Integer = {int_or_zero(solver.get('sol'))};")
    a("    }")
    a("")

    a("    part structuralSummary {")
    a(f"      attribute nodeCount : Integer = {int_or_zero(nodes.get('count'))};")
    a(f"      attribute elementCountsByType : String = {q(element_counts_value)};")
    a("    }")
    a("")
    ...
    for material in materials:
        ...
        a(f"    part {name} {{")
        a(f"      attribute mid : Integer = {int_or_zero(material.get('mid'))};")
        a(f"      attribute type : String = {q(material.get('type'))};")
        a("    }")
        a("")
    ...
    for subcase in subcases:
        ...
        a(f"    part {name} {{")
        a(f"      attribute id : Integer = {int_or_zero(subcase.get('id'))};")
        a(f"      attribute label : String = {q(subcase.get('label'))};")
        a(f"      attribute analysisType : String = {q(subcase.get('analysisType'))};")
        a(f"      attribute spc : String = {q(subcase.get('spc'))};")
        a(f"      attribute load : String = {q(subcase.get('load'))};")
        a("    }")
        a("")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "--output",
        type=Path,
        help="Output SysML path (default: <source_bdf_stem>_presentation.sysml)",
    )
