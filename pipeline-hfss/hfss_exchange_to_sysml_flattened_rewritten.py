...
def emit_sysml(exchange: dict[str, Any]) -> str:
    ...
    a(f"      attribute method : String = {q(solver.get('method'))};")
    a(f"      attribute frequencyRange : String = {q(flatten_frequency(settings.get('frequencyRange', {})))};")
    a("    }")
    a("")
    for idx, mat in enumerate(materials, start=1):
        name = ident(mat.get("id"), f"material_{idx}")
        a(f"    part {name} {{")
        a(f"      attribute id : String = {q(mat.get('id'))};")
        a(f"      attribute type : String = {q(mat.get('type'))};")
        a(f"      attribute properties : String = {q(flatten_material_props(mat.get('properties', {})))};")
        a("    }")
        a("")
    for idx, port in enumerate(ports, start=1):
        name = ident(port.get("id"), f"port_{idx}")
        a(f"    part {name} {{")
        a(f"      attribute id : String = {q(port.get('id'))};")
        a(f"      attribute type : String = {q(port.get('type'))};")
        a(f"      attribute location : String = {q(port.get('location'))};")
        a(f"      attribute impedance : String = {q(port.get('impedance'))};")
        a("    }")
        a("")
    for idx, bc in enumerate(bcs, start=1):
        name = ident(bc.get("id"), f"boundary_{idx}")
        a(f"    part {name} {{")
        a(f"      attribute id : String = {q(bc.get('id'))};")
        a(f"      attribute type : String = {q(bc.get('type'))};")
        a(f"      attribute appliesTo : String = {q(flatten_applies(bc.get('appliesTo', [])))};")
        a("    }")
        a("")
    a("    part provenance {")
    a(f"      attribute sourceFiles : String = {q(flatten_applies(provenance.get('sourceFiles', [])))};")
