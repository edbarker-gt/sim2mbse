...
def emit_sysml(exchange: dict) -> str:
    ...
    a(f"package {pkg} {{")
    a("")
    a("  part def Usb32TypeC {}")
    a("  part def Usb32TypeAStacked {}")
    a("  part def Rj45Ethernet {}")
    a("  part def DisplayPort {}")
    a("  part def M2KeyE {}")
    a("  part def M2KeyM4Lane {}")
    a("  part def M2KeyM2Lane {}")
    a("  part def CameraConnector {}")
    a("  part def ExpansionHeader40Pin {}")
    a("  part def ButtonHeader {}")
    a("  part def CanBusHeader {}")
    a("  part def FanConnector {}")
    a("  part def DcPowerJack {}")
    a("  part def OtherPort {}")
    a("")
    a(f"  part {top} {{")
    a("")
    for p in ports:
        a(f"    part {ident(p.get('id'),'port')} : {p.get('type','OtherPort')};")
    a("")
    a("  }")
    a("}")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create flattened SysML from STEP connector-port exchange JSON.")
